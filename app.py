...
def index():
    ...
    rev_table3 = playstore.groupby(['Category']).agg({
    'Price': 'sum',
    'Reviews': 'sum',
    'Rating': 'mean',
    'Size': 'mean',
    }).sort_values('Rating', ascending=False).reset_index().head()


    X = rev_table3['Rating'].values # axis x
    Y = rev_table3['Category'].values # axis y
    area = rev_table3['Size'].values/10000 
    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
    # isi nama method untuk scatter plot, variabel x, dan variabel y
    plt.scatter(x=X,y=Y, s=area, alpha=0.3)
